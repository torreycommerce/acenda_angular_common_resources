(function (angular){
  ...
  angular.module('material.transitions', ['material.assist'])
...
(function (angular){
  ...
  angular.module('material.transitions')
...
(function (angular){
  ...
  angular.module('material.directives', ['material']);
...
(function (angular){
  ...
  .directive('ngMaterialModal', ['TemplateHandler', '$compile', 'Morph', function (TemplateHandler, $compile, Morph) {
    return {
      restrict: 'A',
      scope: true,
      link: function (scope, element, attrs) {
        var wrapper = angular.element('<div></div>').css('visibility', 'hidden');
        var settings = scope[attrs.ngMaterialModal];
        var isMorphed = false;
        ...
        var initMorphable = function (content) {
          var closeElem  = angular.element(content[0].querySelector(settings.closeElem));
          var elements = {
            morphable: element,
            wrapper: wrapper,
            content: content
...
(function (angular){
  ...
  angular.module('material.directives')
  .directive('ngMaterialOverlay', ['$compile', 'TemplateHandler', 'Morph', function ($compile, TemplateHandler, Morph) {

    return {
      restrict: 'A',
      scope: true,
      link: function (scope, element, attrs) {
        if(typeof scope[attrs.ngMaterialOverlay]  == 'undefined') return;
        var wrapper = angular.element('<div></div>').css('visibility', 'hidden');
        var settings = scope[attrs.ngMaterialOverlay];
        var isMorphed = false;
        ...
        var initMorphable = function (content) {
          var closeElem  = angular.element(content[0].querySelector(settings.closeElem));
          var closeElem2  = angular.element(content[0].querySelector(settings.closeElem2));
          var elements = {
            morphable: element,
            wrapper: wrapper,
            content: content
          ...
          if (closeElem2) {
            closeElem2.bind('click', function (event) {
              settings.MorphableBoundingRect = element[0].getBoundingClientRect();
              isMorphed = overlay.toggle(isMorphed);
            });
          }
...
(function (angular){
  ...
  angular.module('material.assist', [])
